...
#Define prefix mapping with data_type and description
prefix_df <- tribble(
  ~prefix, ~data_type,    ~description,
  ...
  "chr_",  "Character",   "Free text",
  "num_",  "Numeric",     "Raw number",
...
# Define variable groups for data_level
study_vars <- col_names[which(col_names == "chr_authorname") : which(col_names == "is_RCT")]
sample_vars <- c("id_sample", col_names[which(col_names == "chr_grade") : which(col_names == "pct_minority")])
intervention_vars <- c(
  col_names[which(col_names == "id_sample") : which(col_names == "chr_condition_name")],
  col_names[which(col_names == "is_scripted") : which(col_names == "is_fidelity")]
)
es_vars <- c("id_es", col_names[which(col_names == "chr_measure_name") : which(col_names == "r_pre_delay")])

# --- Step 4: Classification function ---
classify_variable <- function(var) {
  matched <- prefix_df %>%
    filter(str_starts(var, prefix)) %>%
    slice(1)
...
df_clean_msdn           <- df_clean %>% select(id_sample, id_condition, cat_condition_type,
                                             any_of(c(es_level_vars))) %>% filter(!is.na(id_es)) %>% filter(!is.na(id_es))
```

```{r}
df_clean_msdn <- df_clean_msdn %>%
...
```{r}
# Pre-test
df_msdn_pre <- match_int_vs_ctl(df_temp_pre) 

# Post-test
df_msdn_post <- match_int_vs_ctl(df_temp_post)

# Delayed-test
df_msdn_delayed <- match_int_vs_ctl(df_temp_delayed)
```

```{r}
# Keep df_msdn_pre as the main table, remove irrelevant columns
df_msdn_combined <- df_msdn_pre %>%
  select(-is_measure_std_ctl)

# Manually select columns from post and delayed tables
df_post_selected <- df_msdn_post %>%
  select(starts_with("m_"), starts_with("sd_"), starts_with("n_"))

df_delayed_selected <- df_msdn_delayed %>%
  select(starts_with("m_"), starts_with("sd_"), starts_with("n_"), starts_with("num_"))

# Bind all together
df_msdn_combined <- bind_cols(df_msdn_combined, df_post_selected, df_delayed_selected)
```
...
dfs_to_save <- list(
  df_clean = df_clean,
  df_clean_msdn = df_clean_msdn, # mean, sd, n
  df_clean_intervention = df_clean_intervention,
  df_clean_sample = df_clean_sample,
  df_clean_study = df_clean_study,
#  df_msdn_pre = df_msdn_pre,
#  df_msdn_post = df_msdn_post,
#  df_msdn_delayed = df_msdn_delayed,
  df_msdn_combined = df_msdn_combined,
